...
%fT4RB
dydt(N) = D*(1.8E-4)*(fT4RB(2)-fT4RB(1))/(dx^2) - k1*fT4RB(1)*TBGRB(1) + k2*T4TBGRB(1) - k3*fT4RB(1)*TTRRB(1) + k4*T4TTRRB(1) - k5*fT4RB(1)*ALBRB(1) + k6*T4ALBRB(1) + (-k21(1)*fT4RB(1) + k28(1)*T4RBT(1)*fuT4RBT)/(VRBB/M) + (fT4B-fT4RB(1))*QRB/(VRBB/M);
dydt(N+M-1) = D*(1.8E-4)*(fT4RB(M-1)-fT4RB(M))/(dx^2) - k1*fT4RB(M)*TBGRB(M) + k2*T4TBGRB(M) - k3*fT4RB(M)*TTRRB(M) + k4*T4TTRRB(M) - k5*fT4RB(M)*ALBRB(M) + k6*T4ALBRB(M) + (-k21(M)*fT4RB(M) + k28(M)*T4RBT(M)*fuT4RBT)/(VRBB/M) + (fT4RB(M-1)-fT4RB(M))*QRB/(VRBB/M);
...
%fT3RB
dydt(N+4*M) = D*(1.8E-4)*(fT3RB(2)-fT3RB(1))/(dx^2) - k7*fT3RB(1)*TBGRB(1) + k8*T3TBGRB(1) - k9*fT3RB(1)*TTRRB(1) + k10*T3TTRRB(1) - k11*fT3RB(1)*ALBRB(1) + k12*T3ALBRB(1) + (-k23(1)*fT3RB(1) + k29(1)*T3RBT(1)*fuT3RBT)/(VRBB/M) + (fT3B-fT3RB(1))*QRB/(VRBB/M);
dydt(N+5*M-1) = D*(1.8E-4)*(fT3RB(M-1)-fT3RB(M))/(dx^2) - k7*fT3RB(M)*TBGRB(M) + k8*T3TBGRB(M) - k9*fT3RB(M)*TTRRB(M) + k10*T3TTRRB(M) - k11*fT3RB(M)*ALBRB(M) + k12*T3ALBRB(M) + (-k23(M)*fT3RB(M) + k29(M)*T3RBT(M)*fuT3RBT)/(VRBB/M) + (fT3RB(M-1)-fT3RB(M))*QRB/(VRBB/M);
...
%T4RBT
dydt(N+11*M) = E*(1.8E-4)*(T4RBT(2)-T4RBT(1))/(dx^2) + (k21(1)*fT4RB(1) - k28(1)*T4RBT(1)*fuT4RBT)/(VRBT/M) - k24(1)*T4RBT(1)*fuT4RBT - k32(1)*T4RBT(1)*fuT4RBT;
dydt(N+12*M-1) = E*(1.8E-4)*(T4RBT(M-1)-T4RBT(M))/(dx^2) + (k21(M)*fT4RB(M) - k28(M)*T4RBT(M)*fuT4RBT)/(VRBT/M) - k24(M)*T4RBT(M)*fuT4RBT - k32(M)*T4RBT(M)*fuT4RBT;

%T3RBT
dydt(N+12*M) = E*(1.8E-4)*(T3RBT(2)-T3RBT(1))/(dx^2) + (k23(1)*fT3RB(1) - k29(1)*T3RBT(1)*fuT3RBT)/(VRBT/M) + k24(1)*T4RBT(1)*fuT4RBT - k33(1)*T3RBT(1)*fuT3RBT;
dydt(N+13*M-1) = E*(1.8E-4)*(T3RBT(M-1)-T3RBT(M))/(dx^2) + (k23(M)*fT3RB(M) - k29(M)*T3RBT(M)*fuT3RBT)/(VRBT/M) + k24(M)*T4RBT(M)*fuT4RBT - k33(M)*T3RBT(M)*fuT3RBT;
...
%fT4L
dydt(N+16*M) = D*(1.8E-4)*(fT4L(2)-fT4L(1))/(dx^2) - k1*fT4L(1)*TBGL(1) + k2*T4TBGL(1) - k3*fT4L(1)*TTRL(1) + k4*T4TTRL(1) - k5*fT4L(1)*ALBL(1) + k6*T4ALBL(1) + (-k25(1)*fT4L(1) + k30(1)*T4LT(1)*fuT4LT)/(VLB/M) + (fT4B-fT4L(1))*QL/(VLB/M);
dydt(N+17*M-1) = D*(1.8E-4)*(fT4L(M-1)-fT4L(M))/(dx^2) - k1*fT4L(M)*TBGL(M) + k2*T4TBGL(M) - k3*fT4L(M)*TTRL(M) + k4*T4TTRL(M) - k5*fT4L(M)*ALBL(M) + k6*T4ALBL(M) + (-k25(M)*fT4L(M) + k30(M)*T4LT(M)*fuT4LT)/(VLB/M) + (fT4L(M-1)-fT4L(M))*QL/(VLB/M);
...
%fT3L
dydt(N+20*M) = D*(1.8E-4)*(fT3L(2)-fT3L(1))/(dx^2) - k7*fT3L(1)*TBGL(1) + k8*T3TBGL(1) - k9*fT3L(1)*TTRL(1) + k10*T3TTRL(1) - k11*fT3L(1)*ALBL(1) + k12*T3ALBL(1) + (-k27(1)*fT3L(1) + k31(1)*T3LT(1)*fuT3LT)/(VLB/M) + (fT3B-fT3L(1))*QL/(VLB/M);
dydt(N+21*M-1) = D*(1.8E-4)*(fT3L(M-1)-fT3L(M))/(dx^2) - k7*fT3L(M)*TBGL(M) + k8*T3TBGL(M) - k9*fT3L(M)*TTRL(M) + k10*T3TTRL(M) - k11*fT3L(M)*ALBL(M) + k12*T3ALBL(M) + (-k27(M)*fT3L(M) + k31(M)*T3LT(M)*fuT3LT)/(VLB/M) + (fT3L(M-1)-fT3L(M))*QL/(VLB/M);
...
%T4LT
dydt(N+27*M) = E*(1.8E-4)*(T4LT(2)-T4LT(1))/(dx^2) + (k25(1)*fT4L(1) - k30(1)*T4LT(1)*fuT4LT)/(VLT/M) - k26(1)*T4LT(1)*fuT4LT - k34(1)*T4LT(1)*fuT4LT;
dydt(N+28*M-1) = E*(1.8E-4)*(T4LT(M-1)-T4LT(M))/(dx^2) + (k25(M)*fT4L(M) - k30(M)*T4LT(M)*fuT4LT)/(VLT/M) - k26(M)*T4LT(M)*fuT4LT - k34(M)*T4LT(M)*fuT4LT;

%T3LT
dydt(N+28*M) = E*(1.8E-4)*(T3LT(2)-T3LT(1))/(dx^2) + (k27(1)*fT3L(1) - k31(1)*T3LT(1)*fuT3LT)/(VLT/M) + k26(1)*T4LT(1)*fuT4LT - k35(1)*T3LT(1)*fuT3LT;
dydt(N+29*M-1) = E*(1.8E-4)*(T3LT(M-1)-T3LT(M))/(dx^2) + (k27(M)*fT3L(M) - k31(M)*T3LT(M)*fuT3LT)/(VLT/M) + k26(M)*T4LT(M)*fuT4LT - k35(M)*T3LT(M)*fuT3LT;
...
for i=2:(M-1)
    dydt(i-1+N) = D*(1.8E-4)*(fT4RB(i+1)+fT4RB(i-1)-2*fT4RB(i))/(dx^2) - k1*fT4RB(i)*TBGRB(i) + k2*T4TBGRB(i) - k3*fT4RB(i)*TTRRB(i) + k4*T4TTRRB(i) - k5*fT4RB(i)*ALBRB(i) + k6*T4ALBRB(i) + (-k21(i)*fT4RB(i) + k28(i)*T4RBT(i)*fuT4RBT)/(VRBB/M) + ((fT4RB(i-1)-fT4RB(i))*QRB)/(VRBB/M);
    ...
    dydt(i+4*M-1+N) = D*(1.8E-4)*(fT3RB(i+1)+fT3RB(i-1)-2*fT3RB(i))/(dx^2) - k7*fT3RB(i)*TBGRB(i) + k8*T3TBGRB(i) - k9*fT3RB(i)*TTRRB(i) + k10*T3TTRRB(i) - k11*fT3RB(i)*ALBRB(i) + k12*T3ALBRB(i) + (-k23(i)*fT3RB(i) + k29(i)*T3RBT(i)*fuT3RBT)/(VRBB/M) + (fT3RB(i-1)-fT3RB(i))*QRB/(VRBB/M);
    ...
    dydt(i+11*M-1+N) = E*(1.8E-4)*(T4RBT(i+1)+T4RBT(i-1)-2*T4RBT(i))/(dx^2) + (k21(i)*fT4RB(i) - k28(i)*T4RBT(i)*fuT4RBT)/(VRBT/M) - k24(i)*T4RBT(i)*fuT4RBT - k32(i)*T4RBT(i)*fuT4RBT;
    dydt(i+12*M-1+N) = E*(1.8E-4)*(T3RBT(i+1)+T3RBT(i-1)-2*T3RBT(i))/(dx^2) + (k23(i)*fT3RB(i) - k29(i)*T3RBT(i)*fuT3RBT)/(VRBT/M) + k24(i)*T4RBT(i)*fuT4RBT - k33(i)*T3RBT(i)*fuT3RBT;
    ...
    dydt(i+16*M-1+N) = D*(1.8E-4)*(fT4L(i+1)+fT4L(i-1)-2*fT4L(i))/(dx^2) - k1*fT4L(i)*TBGL(i) + k2*T4TBGL(i) - k3*fT4L(i)*TTRL(i) + k4*T4TTRL(i) - k5*fT4L(i)*ALBL(i) + k6*T4ALBL(i) + (-k25(i)*fT4L(i) + k30(i)*T4LT(i)*fuT4LT)/(VLB/M) + (fT4L(i-1)-fT4L(i))*QL/(VLB/M);
    ...
    dydt(i+20*M-1+N) = D*(1.8E-4)*(fT3L(i+1)+fT3L(i-1)-2*fT3L(i))/(dx^2) - k7*fT3L(i)*TBGL(i) + k8*T3TBGL(i) - k9*fT3L(i)*TTRL(i) + k10*T3TTRL(i) - k11*fT3L(i)*ALBL(i) + k12*T3ALBL(i) + (-k27(i)*fT3L(i) + k31(i)*T3LT(i)*fuT3LT)/(VLB/M) + (fT3L(i-1)-fT3L(i))*QL/(VLB/M);
    ...
    dydt(i+27*M-1+N) = E*(1.8E-4)*(T4LT(i+1)+T4LT(i-1)-2*T4LT(i))/(dx^2) + (k25(i)*fT4L(i) - k30(i)*T4LT(i)*fuT4LT)/(VLT/M) - k26(i)*T4LT(i)*fuT4LT - k34(i)*T4LT(i)*fuT4LT;
    dydt(i+28*M-1+N) = E*(1.8E-4)*(T3LT(i+1)+T3LT(i-1)-2*T3LT(i))/(dx^2) + (k27(i)*fT3L(i) - k31(i)*T3LT(i)*fuT3LT)/(VLT/M) + k26(i)*T4LT(i)*fuT4LT - k35(i)*T3LT(i)*fuT3LT;
